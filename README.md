...
В текущей версии, контент статически содержится в папке /assets в виде файлов и директорий по годам, месяца и дням. Один день - один XML файл, весь контент содержится в нем. Дни группированы по месяцам, а месяцы по годам.

`TO-DO Контент будет перенесен на удаленный репозиторий. Предварительно на Firebase. Интерфейс репозитория останется неизменным` 
...
Есть всего одна переменная хранящее состояние - `DateTime _currentDate`. Все приложение реагирует на его изменение и рисует себя в зависимости от его значения. Значение - это дата текущего дня, который должно быть отобразиться на экране.

Есть всего несколько методов изменяющих `_currentDate`:
- При инициализации ставится сегодняшняя дата
- При нажатии стрелок вправо и влево, соответственно дата смещается на один день
- Через DatePicker. Диалог выбора дня по календарю
